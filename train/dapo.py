...
def train():
    ...
    # 检查：mini_batch_size 必须是 num_processes 的倍数
    if mini_batch_size % num_processes != 0:
        raise ValueError(
            f"mini_batch_size ({mini_batch_size}) must be a multiple of num_processes ({num_processes}). "
            f"This ensures each rank processes the same amount of data."
        )
    ...
    # 计算有效的梯度累积步数（考虑数据扩展）
    # 每个rank在rollout时分配到 (mini_batch_size / num_processes) 个task
    # 每个task有 num_response_per_task 个response，经过expansion后变成多个训练样本
    # 梯度累积步数 = (mini_batch_size * num_response_per_task / num_processes) * expansion_factor
    ...
    print(f"  - Mini batch size (tasks): {mini_batch_size}")
    ...
    for step in range(config.experiment.current_step, config.training.num_train_steps+1):
        ...
        ##################################
        #        准备训练输入 - 每个device处理本地response #
        ##################################
        # 直接使用本地数据，不进行gather和redistribute
        prompt_ids_list, token_ids_list, reward_list, raw_reward_list, rollout_step_maps = [], [], [], [], []
        
        for x in dataset_load:
            prompt_ids_list.extend(x["prompt_ids"])
            token_ids_list.extend(x["token_ids"])
            reward_list.extend(x["rewards"])
            raw_reward_list.extend(x["raw_rewards"])
            
            if "step_map" in x.keys() and x['step_map'] is not None:
                rollout_step_maps.extend(x["step_map"])
            else:
                for response_ids in x["token_ids"]:
                    rollout_step_maps.append(list(range(1, len(response_ids) + 1)))
        
        logger.info(f"[Rank {accelerator.process_index}] Processing {len(prompt_ids_list)} local responses")
        accelerator.wait_for_everyone()
        ...
        # 计算DAPO的group累积步数
        # 每个group = 一个task的所有responses
        # steps_per_group = gradient_accumulation_steps / (num_task_per_step / mini_batch_size)
        steps_per_group = gradient_accumulation_steps // (num_task_per_step // mini_batch_size)
        num_groups_per_update = gradient_accumulation_steps // steps_per_group
        logger.info(f"[Rank {accelerator.process_index}] DAPO Configuration:")
        logger.info(f"[Rank {accelerator.process_index}]   Steps per group: {steps_per_group} = {gradient_accumulation_steps} / ({num_task_per_step} / {mini_batch_size})")
        logger.info(f"[Rank {accelerator.process_index}]   Number of groups per update: {num_groups_per_update}")
        
        # DAPO: 预先计算每个group的总token数（除以block_size）
        group_token_counts = []
        for group_idx in range(0, num_forward_batches, steps_per_group):
            group_start = group_idx * batch_size_train
            group_end = min((group_idx + steps_per_group) * batch_size_train, total_expanded_samples)
            group_labels = expanded_labels[group_start:group_end]
            # 扩展后的token数需要除以block_size得到真实token数
            expanded_tokens = (group_labels != -100).sum().item()
            real_tokens = expanded_tokens / block_size
            group_token_counts.append(real_tokens)
            logger.info(f"[Rank {accelerator.process_index}] Group {len(group_token_counts)-1}: samples [{group_start}:{group_end}], expanded_tokens={expanded_tokens}, real_tokens={real_tokens:.0f}")
        
        logger.info(f"[Rank {accelerator.process_index}] Pre-calculated {len(group_token_counts)} groups with token counts")
        ...
        for iteration in range(num_iterations):
            ...
            for bstep, batch in progress_bar:
                ...
                # DAPO: 确定当前batch所属的group，并获取该group的总token数
                group_idx = bstep // steps_per_group
                group_tokens = group_token_counts[group_idx]
                
                # 计算当前batch的真实token数
                batch_real_tokens = (labels != -100).sum().item() / block_size
                
                # 累积统计（用于logging）
                mb_loss += loss.item()
                mb_tokens += batch_real_tokens
                mb_count += 1
                
                # 检查是否完成了一个group的累积
                group_step = bstep % steps_per_group
                is_group_end = ((group_step + 1) == steps_per_group) or ((bstep + 1) == num_forward_batches)
                
                # DAPO: 每个batch的loss除以group总token数，再除以总group数进行归一化
                normalized_loss = loss / max(group_tokens, 1) / num_groups_per_update
                
                # Backward（DeepSpeed会自动处理梯度累积和同步）
                accelerator.backward(normalized_loss)
                
                # 检查是否到达梯度累积结束点
                is_accum_end = ((bstep + 1) % gradient_accumulation_steps == 0) or ((bstep + 1) == num_forward_batches)
                ...
                if is_accum_end:
                    ...
                    if accelerator.is_main_process:
                        ...
                        avg_loss = mb_loss / max(mb_tokens, 1)
                        print(f"[Optimizer Step {optimizer_step}] lr={current_lr}, grad_norm={grad_str}, loss={avg_loss:.6f}")
                # 统计当前batch
                ...
                if should_log:
                    # 聚合所有rank的统计值（包含真实token数）
                    mb_stats = torch.tensor([mb_loss, mb_reward, mb_entropy, mb_kl, float(mb_count), mb_tokens], device=accelerator.device)
                    all_stats = accelerator.gather(mb_stats).cpu().view(accelerator.num_processes, -1)
                    
                    # 计算全局平均（loss用真实token数计算）
                    global_loss = all_stats[:, 0].sum() / all_stats[:, 5].sum()
